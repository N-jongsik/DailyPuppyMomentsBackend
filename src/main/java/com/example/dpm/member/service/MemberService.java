...
public class MemberService {
	...
	private MemberDto toDto(MemberEntity memberEntity) {
		return MemberDto.builder().member_id(memberEntity.getMember_id()).socialId(memberEntity.getSocialId())
				.nickname(memberEntity.getNickname()) // 필드명 수정
				.profile_image(memberEntity.getProfile_image()).point(memberEntity.getPoint())
				.attendance(memberEntity.isAttendance()).is_deleted(memberEntity.is_deleted())
	...
	public MemberEntity toEntity(MemberDto memberDto) {
		return MemberEntity.builder().member_id(memberDto.getMember_id()).socialId(memberDto.getSocialId())
				.nickname(memberDto.getNickname()).profile_image(memberDto.getProfile_image())
				.point(memberDto.getPoint()).attendance(memberDto.isAttendance()).is_deleted(memberDto.is_deleted())
	...
	public void saveTokens(Long member_id, String accessToken, String refreshToken) {
		...
		if (memberOptional.isPresent()) {
			...
			memberEntity = MemberEntity.builder().member_id(member_id).build();
		}
